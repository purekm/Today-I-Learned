...
=> 병렬로 처리하다보니까 처리량은 올라갈텐데, 동기화가 복잡해 질 수 있다는 단점이 존재

#### Multicore Programming
### 병렬 처리 vs 병행 처리
- **병렬 처리(Parallelism)**: 여러 코어가 물리적으로 동시에 실행
- **병행 처리(Concurrency)**: 둘 이상의 작업이 진행 중인 상태 (스케줄링 및 시분할을 통해 동시에 진행중인 것 처럼 보이게 구조화)
