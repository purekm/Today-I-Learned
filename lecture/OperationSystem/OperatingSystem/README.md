...
가상화랑 컨테이너는 무슨 차이가 있을까?
=> 가상화는 여러 OS를 실행시킬 수 있는데, 컨테이너는 호스트 OS의 커널을 빌려서 사용하고 
OS를 포함하지 않기 때문에 가볍고 빠름(가상화는 여러 PC를 사용하는 느낌이고, 컨테이너는 하나의 PC에서 여러 계정을 사용하는 느낌)

### 분산 시스템 (Distributed Systems)
- **분산 시스템**은 네트워크로 연결된 여러 독립적인 컴퓨터 시스템들이 협력하여 작업을 수행하는 시스템임
- 이러한 시스템은 고가용성을 제공하며, 클러스터링 기술을 통해 서버가 고장 나더라도 서비스를 지속할 수 있음
- 분산 시스템은 다양한 물리적 위치에 있는 컴퓨터들이 네트워크를 통해 상호작용하면서 하나의 시스템처럼 동작함

---

## 커널
- 커널은 OS의 핵심 부분으로, 하드웨어와 프로그램 사이에서 중간다리 역할을 하는 인터페이스를 제공

프로세스 관리 (스케줄링), 메모리 관리(다른 프로그램의 메모리와 겹치지 않게), I/O 관리등등 여러 역할을 하는데, 커널은 OS의 중요한 부분이기 때문에 보호해야 함
-> 사용자 모드와 커널모드로 분리 함으로써, 사용자 모드에서 시스템자원에 관련된 접근을 하고싶으면 system call을 통해서 커널 모드로 접근해야 함   
---