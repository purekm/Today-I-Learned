...
Usermode에서 open같은 함수를 호출하면, Trap 변화로 user mode -> kernel mode로 변경
이후 open 함수가 systemcall 요청하고, 요청된 작업이 끝나면 다시 kernel mode -> usermode로 복구
...
=> 링커에서 여러 라이브러리를 가져와서 하나의 완전한 실행파일로 만들고, 로더를 통해 만든 실행파일을 메모리에 적재하고 실행준비

현대 범용 시스템에서는 라이브러리를 실행 파일에 포함시키지 않고, 필요할 때 동적으로 링크된 라이브러리를 로드함. (**동적링킹**)
1. 링커가 라이브러리를 포함시키지 않고, 해당 프로그램을 실행시키려면 어떤 라이브러리의 어떤 함수가 필요하다는 식으로 메모를 남겨둠
2. 로더가 프로그램을 메모리에 올릴때 이 메모를 보고, 메모리에 함께 올려줌(함수호출 시) 
...
### Crash Dump
운영 체제 자체의 오류가 발생하면 **커널 메모리**를 캡처한 크래시 덤프 파일 생성.
